...
- 초 단위로 작업을 나눠 측정
- 보통 작업의 동시 흐름을 보여주는 프로그램이 있는 경우, 가능한 빨리 많은 작업을 수행하기 원한다. 이런 경우 처리량이 적절한 성능 측정 기준이 된다.

<br/>
처리량을 개선하는 방법에는 두 가지가 있다.

### 1. 작업을 여러 하위 작업으로 나눈다.

- Throughput = 1 / T
  - 작업을 완수하는데 T 시간이 걸릴 때 완료할 수 있는 처리량 -> 최소 처리량
- Throughput = N / T (theoretical)
  - N: 하위 작업 수, T: 원래 작업을 실행하는 시간
  - 작업을 나눠서 실행했을 때의 처리량 -> 최대 처리량
- Throughput < N / T (in practice)
  - 지연 시간 최적화 같은 문제가 원인
  
    -> 원래 작업을 여러 하위 작업으로 나누고 스케줄링한 후 다시 결합하는 데에 비용이 발생하는데 이러한 비용은 처리량에 있어 불필요한 작업이다.

<br/>

### 2. 각 작업을 별개 스레드에 할당한다.
- Throughput = N / T (theoretical)
  - 1번 방법과는 다르게 각 작업의 수행 지연 시간을 최소화하므로 이론상 얻을 수 있는 최대 처리량에 가까운 수치를 얻을 수 있다.
    
    -> 작업이 내부적으로 연관되지 않고 독립적이기 때문
    - 각 작업을 작은 작업으로 나누는 전처리 비용 없음
    - 독립적인 작업이기 때문에 다른 작업이 완료될 때까지 기다릴 필요 없음
    - 각 작업의 결과를 하나의 결과로 합치는 후처리 비용 없음
  - 스레드 풀링이나 캐시 친화적인 논블로킹 큐 같은 기술을 사용하면 거의 최대 처리량을 얻을 수 있다.

<br/>

### Thread Pooling
> 미리 스레드를 생성하고 생성된 스레드를 재사용할 수 있는 것
- 매번 다시 스레드를 생성하지 않아도 된다.
- 일정한 수의 스레드를 유지할 수 있다.
  - 동작 원리
  1. 스레드가 생성되면 풀에 쌓이고, 작업이 큐를 통해 각각의 스레드에 할당된다. 
  2. 스레드는 이용 가능한 상태라면 큐를 통해 작업을 받는다. 
  3. 모든 스레드가 작업 중이라면 작업들은 스레드가 이용 가능할 때까지 큐에서 대기한다.

<br/>

### 단어 수 세기 예시
단어 수 세기 예시에서 알 수 있듯이, 물리적인 코어 수 만큼 스레드를 늘리면 처리량이 급격히 늘어나는 것을 확인할 수 있다.

가상 코어 수 만큼 스레드를 늘리면 처리량도 어느 정도 꾸준히 증가한다.

그러나 그 이상으로 늘리면 처리량은 더 이상 늘어나지 않는다.

이미지 프로세싱 예시와 다르게 가상 코어 수 만큼 스레드를 늘려도 처리량이 좋아지는 것을 볼 수 있는데 이는 원래 작업을 여러 하위 작업으로 나누고 스케줄링한 후 다시 결합하는 데에 발생하는 비용을 제거했기 때문이다. 
